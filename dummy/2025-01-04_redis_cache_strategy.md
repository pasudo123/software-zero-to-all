...
## Lazy Loading
- 데이터 요청이 있을 때만 캐싱한다. 즉 최소 1번은 원본 데이터를 조회해야 캐시에 적재된다.
...
## Write Through
- 쓰기 요청이 캐시와 데이터베이스에 동기적으로 반영되는 패턴이다.
- 캐시와 데이터베이스 간의 데이터 일관성을 높일 수 있다.
- 쓰기 경로에 캐시와 데이터베이스 반영이 모두 포함되므로 쓰기 지연이 늘어날 수 있다.
- 데이터베이스 반영을 비동기로 미루면 write-through 가 아니라 write-behind 성격에 가까워진다.
```kotlin
