...
- `UncheckedException` 은 `run-time exception 클래스` 및 `error 클래스` 이다.
- `CheckedException` 은 unchecked exception 을 제외한 모든 exception 클래스다.
...
class Demo01ServiceWithRollback(
  ...
  fun queryWithSize(query: Query, size: Int) {
    ...
    /**
     * @Transactional(rollbackFor = [Exception::class]) 설정이 필요하다.
     * 없으면 롤백이 되지 않는다.
     */
