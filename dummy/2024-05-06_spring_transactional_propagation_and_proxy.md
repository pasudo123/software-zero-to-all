...
Spring 의 기본 proxy mode 에서는 프록시를 통과한 외부 호출만 트랜잭션 인터셉터가 적용된다. 같은 클래스 내부에서 `innerCreateIndependently()` 를 직접 호출하면 `REQUIRES_NEW` 설정 자체가 적용되지 않고, 외부에서 호출된 `create()` 의 트랜잭션 안에서 함께 실행된다. 따라서 `create()` 에서 RuntimeException 이 발생하면 함께 롤백된다.
```kotlin
@Service
class PostService(
...
- self-injection 으로 자기 자신의 프록시를 주입받는 방법도 있지만, 순환참조나 설계 복잡도가 생길 수 있어 별도 서비스로 분리하는 편이 명확하다.
- 같은 클래스 내부 호출까지 트랜잭션을 적용해야 한다면 Spring 문서 기준으로 AspectJ mode 를 고려할 수 있다.
